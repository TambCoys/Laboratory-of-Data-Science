...
for t in tracks:
    ...
    if date_key not in date_lookup:
        ...
        try:
            ...
            day_str = f"{int(y_raw):04d}{int(m_raw):02d}{int(d_raw):02d}" if (
                        y_raw is not None and m_raw is not None and d_raw is not None) else None
        except:
            year_str, month_str, day_str = None, None, None
        season = t.get("season")
        DateDim.append([pk_date, day_str, year_str, month_str, season])
    else:
